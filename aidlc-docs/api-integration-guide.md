...
| API | 소스 | 데이터 | 동기화 주기 | 상태 |
|-----|------|--------|-------------|------|
| 중소벤처24 (SMES) | smes24 | 정부지원사업 공고 | 매일 00:00, 12:00 | ✅ 완료 |
| 기업마당 (bizinfo) | bizinfo | 중기부 지원사업 | 매일 01:00, 13:00 | ✅ 완료 |
| K-Startup | kstartup | 창업 지원사업 | 매일 02:00, 14:00 | ✅ 완료 |
| 나라장터 (G2B) | g2b | 조달청 입찰공고 | 매일 03:00, 15:00 | ⚠️ 401 오류 |
| HRD Korea | hrd | 직업훈련 과정 | 매일 04:00, 16:00 | ⚠️ API 키 필요 |
| 국세청 | nts | 사업자등록정보 검증 | 실시간 | ✅ 완료 |
| 국민연금공단 | nps | 사업장 정보 조회 | 월간 CSV | ✅ 완료 |

### 내부 API

| API | 엔드포인트 | 설명 | 상태 |
|-----|-----------|------|------|
| 공개 통계 | `/api/statistics` | 랜딩 페이지용 실시간 통계 | ✅ 완료 |
| UTM 분석 | `/api/admin/utm` | 마케팅 채널별 분석 (관리자) | ✅ 완료 |
| 피드백 수집 | `/api/feedback` | 사용자 피드백 수집/관리 | ✅ 완료 |
| 비회원 매칭 | `/api/guest/matching` | 비회원 AI 매칭 플로우 | ✅ 완료 |
| 벡터 임베딩 | `/api/embeddings/generate` | RAG 시맨틱 검색용 임베딩 | ✅ 완료 |
| 기업정보 조회 | `/api/business/lookup` | 사업자번호로 기업정보 자동조회 | ✅ 완료 |
...
```json
{
  ...
  "data": {
    ...
    "bySource": [
      { "name": "google", "total": 50, "converted": 20, "conversionRate": 40.0 }
    ],
    "byMedium": [...],
    "byCampaign": [...],
    "dailyTrend": [...]
...
### 엔드포인트
```
POST /api/guest/matching           # 매칭 분석 요청
GET  /api/guest/matching/[id]      # 결과 조회 (1~2순위 블러)
```
...
## 11. 벡터 임베딩 API (관리자 전용)

### 개요
- **용도**: RAG 시맨틱 검색을 위한 공고 벡터화
- **기술**: Gemini text-embedding-004 (768차원) + pgvector

### 엔드포인트
```
POST /api/embeddings/generate      # 임베딩 생성 (배치)
GET  /api/embeddings/generate      # 임베딩 현황 조회
```
...
### 개요
- **용도**: 사업자번호로 기업정보 자동 조회
- **데이터**: 국민연금공단 사업장 데이터

### 엔드포인트
```
GET /api/business/lookup?businessNumber=1234567890
```
...
| API | 제공처 | 데이터 | 상태 |
|-----|--------|--------|------|
| 나라장터 (G2B) | 조달청 | 입찰공고 | API 키 재발급 필요 |
| HRD Korea | 한국고용정보원 | 직업훈련 | API 키 설정 필요 |
