...
export class BusanScraper {
  ...
  // TODO: 실제 부산시 지원사업 API 또는 웹페이지 URL로 교체
  private readonly BASE_URL = 'https://www.busan.go.kr'
  // 예시 엔드포인트 (실제로는 부산시 공공데이터 API 또는 RSS 피드)
  // private readonly API_URL = 'https://data.busan.go.kr/api/support-programs'
  ...
  private async scrapeWebPage(limit: number, daysBack: number): Promise<ScraperResult> {
    ...
    try {
      ...
      console.error('[부산시 웹 스크래핑] 오류:', error)
      return {
        announcements: [],
        total: 0,
        source: 'local_busan',
  ...
  private parseApiResponse(data: any): ScraperResult {
    ...
    for (const item of items) {
      announcements.push({
        source_id: item.id || item.noticeId || `BUSAN_${Date.now()}`,
        title: item.title || item.noticeName,
        organization: item.organization || '부산광역시',
        category: '지자체',
        support_type: '부산시',
