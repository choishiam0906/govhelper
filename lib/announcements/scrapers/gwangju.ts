 ...
 *
 * 데이터 소스:
 * - 광주광역시 중소기업 지원사업 공고 페이지
 * - URL: https://www.gwangju.go.kr
 *
 * TODO: 실제 API 또는 RSS 피드 URL로 교체 필요
 * - 광주시 공공데이터 포털: https://data.gwangju.go.kr
 * - 광주시청 지원사업 페이지 HTML 스크래핑
 *
 ...
 * 광주광역시 스크래퍼
 */
export class GwangjuScraper {
  readonly id = 'gwangju'
  readonly name = '광주광역시'

  // TODO: 실제 광주시 지원사업 API 또는 웹페이지 URL로 교체
  private readonly BASE_URL = 'https://www.gwangju.go.kr'
  // 예시 엔드포인트 (실제로는 광주시 공공데이터 API 또는 RSS 피드)
  // private readonly API_URL = 'https://data.gwangju.go.kr/api/support-programs'
  ...
  async scrape(options?: ScraperOptions): Promise<ScraperResult> {
    ...
    try {
      ...
      // const apiKey = process.env.GWANGJU_DATA_API_KEY
      ...
      // const feed = await parser.parseURL('광주시_RSS_URL')
      // return this.parseRssFeed(feed.items, limit)

      // 방법 3: HTML 스크래핑 (cheerio 사용)
      // return await this.scrapeWebPage(limit, daysBack)

      console.log('[광주시 스크래퍼] 실제 API/RSS 엔드포인트 설정 필요')

      // 임시: 빈 결과 반환
      return {
        announcements: [],
        total: 0,
        source: 'local_gwangju',
      }

    } catch (error) {
      console.error('[광주시 스크래퍼] 오류:', error)
      return {
        announcements: [],
        total: 0,
        source: 'local_gwangju',
    ...
    try {
      ...
      $('.announcement-list .item').each((index, element) => {
        ...
        if (title && applicationEnd) {
          announcements.push({
            source_id: `GWANGJU_${Date.now()}_${index}`,
            title,
            organization,
            category: '지자체',
            support_type: '광주시',
      ...
      console.error('[광주시 웹 스크래핑] 오류:', error)
      return {
        announcements: [],
        total: 0,
        source: 'local_gwangju',
  ...
  private parseApiResponse(data: any): ScraperResult {
    ...
    for (const item of items) {
      announcements.push({
        source_id: item.id || item.noticeId || `GWANGJU_${Date.now()}`,
        title: item.title || item.noticeName,
        organization: item.organization || '광주광역시',
        category: '지자체',
        support_type: '광주시',
 ...
 * 광주시 스크래퍼 싱글톤 인스턴스
 */
export const gwangjuScraper = new GwangjuScraper()
