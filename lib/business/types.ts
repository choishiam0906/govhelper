...
export interface EmploymentInsuranceResult {
  ...
  businessNumber: string;
  companyName: string;
  businessType: string; // 사업장 업종
  totalInsured: number; // 고용보험 가입자 수
  joinDate: string | null; // 가입일
...
export interface BusinessLookupResult {
  ...
  partialResults?: {
    ...
    employmentInsurance?: EmploymentInsuranceResult | null;
  };
