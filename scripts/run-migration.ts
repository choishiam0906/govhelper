...
// 마이그레이션 파일 경로
const MIGRATION_FILE = path.join(__dirname, '..', 'supabase', 'migrations', '006_nps_business_registry.sql')
...
async function runMigration() {
  ...
  for (let i = 0; i < statements.length; i++) {
    ...
    // COMMENT 문은 건너뛰기 (supabase-js에서 지원 안함)
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message.includes('function') || error.message.includes('does not exist')) {
          // REST API로 직접 실행 불가 - 대시보드에서 실행 필요
