...
| 번호 | 작업 | 설명 | 상태 |
|------|------|------|------|
| 8 | GA4 전환 퍼널 | 비회원→회원→구독 전환 추적 | 대기 |
| 9 | AI 프롬프트 버전 관리 | A/B 테스트 가능한 버전 관리 시스템 | 대기 |
| 10 | 하이브리드 검색 | 벡터 + 키워드 결합 (재현율 +15%) | 대기 |
| 11 | 비회원 플로우 분석 | Step별 포기율 GA4 추적 | 대기 |
| 12 | 가격 A/B 테스트 | Pro ₩3,900 vs ₩5,000 테스트 | 대기 |
| 13 | 이메일 재타겟팅 | 7일 후 마감 임박 알림 자동화 | 대기 |

#### P3 - 중장기 개선 (1개월+)
| 번호 | 작업 | 설명 | 상태 |
|------|------|------|------|
| 14 | 결제 방법 확대 | Naver Pay, Kakao Pay 추가 | 대기 |
| 15 | 팀 플랜 | ₩30,000/월 (3-5명) | 대기 |
| 16 | 데이터 소스 확대 | 은행 정책금융, 지자체 지원금 | 대기 |
...
- `f7b5cd3` OG 이미지 다크 테마로 개선 - 사이트 UX/UI 통일

**7. OG 이미지 다크 테마 적용**
- 배경색: 파란 그라디언트 → 검정색 (#0a0a0a)
- 로고: 사이트와 동일하게 검정 배경 + 흰색 G
- 텍스트/뱃지: 다크 테마에 맞게 수정
- `--force` 옵션으로 Vercel 캐시 갱신 배포 완료
